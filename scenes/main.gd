...
func start_client(player_session_id: String, ip_address: String, port: int) -> bool:
	var peer: ENetMultiplayerPeer = ENetMultiplayerPeer.new()
	...
	multiplayer.multiplayer_peer = peer
	multiplayer.connected_to_server.connect(_on_connected_to_server.bind(player_session_id), CONNECT_ONE_SHOT)
...
func _on_connected_to_server(player_session_id: String):
	send_player_session_id.rpc_id(1, player_session_id)


signal on_player_connected(player_session_id: String, id: int)


@rpc("any_peer", "call_remote", "reliable")
func send_player_session_id(player_session_id: String):
	var accept_player_session_result: AmazonGameLiftServer_Outcome = AmazonGameLiftServer_Outcome.new()
	AmazonGameLiftServer_GameLiftServerAPI.accept_player_session(player_session_id, accept_player_session_result)
	
	var sender_id = multiplayer.get_remote_sender_id()
	
	if not accept_player_session_result.success:
		notification.emit(NotificationLevel.Error, "accept player session failed: %s" % accept_player_session_result.error_message)
		multiplayer.multiplayer_peer.disconnect_peer(sender_id, true)
		return
		
	on_player_connected.emit(player_session_id, sender_id)


func on_player_disconnected(player_session_id: String):
	var remove_player_session_result: AmazonGameLiftServer_Outcome = AmazonGameLiftServer_Outcome.new()
	AmazonGameLiftServer_GameLiftServerAPI.remove_player_session(player_session_id, remove_player_session_result)
			
	if not remove_player_session_result.success:
		notification.emit(NotificationLevel.Error, "remove player session failed: %s" % remove_player_session_result.error_message)
...
func enter_game() -> bool:
	...
	if !start_client(outcome.result.player_session.player_session_id, outcome.result.player_session.ip_address, outcome.result.player_session.port):
		get_tree().quit()
		return false
